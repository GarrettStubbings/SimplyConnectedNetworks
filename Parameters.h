...
namespace Parameters {
    ...
    double pRewire; // Fraction of Network Size to Rewire every Generation
    std::string evoCondition; //what measure of fitness
    double growthRate; //Growth rate used in Evolutionary Entropy
    int numThreads; //Number of threads used in parallel implementation
    double mu; //Degree exponent in mortality calculation
    double gammaD; //death rate constant
    double lambda; // Merit Function weighting
    double beta; // Annealing temperature offset (really sets scale, 100 -> 1%)
    double power; // Power law scaling of inverse temperature
    double runHours; //hours to run the evolution
    int numChanges; // number of moves deltaP to change correlations
    std::string initialDistribution; // FileName of InitialDistribution
                                     // NO EXTENSIONS, Implied CSV.
...
void SetReducedParameters(int argc, char *argv[]) {
...
//All global parameters used (Hopefully).
//Default parameter set for testing purposes
void SetDefaultParameters() {
    
    using namespace Parameters;

    //Variable parameters
    R = 3.0; //Ratio of damage to repair
    gammap = 7.5; //Damage rate constant
    gamman = 6.5; //repair rate constan
    mu = 0.0; // degree weighting in proportional hazards
    gammaD = 8.0; // Factor in proportional Hazards
    numThreads = 1;
    Topology = "Multiple";
    nd = 2;
    alpha = 2.35;
    MortalityCondition = "AND";
    NetworkType = "Bertotti";
...
void SetEvolutionParameters(int argc, char *argv[]) {
	...
	SingleSeed = atoi(argv[14]);  std::cout << ", SingleSeed: " << SingleSeed;
        p_assortativity = atof(argv[15]);  std::cout << ", p_assortativity: " << p_assortativity;
        pRewire = atof(argv[16]); std::cout << ", pRewire: " << pRewire;
        runHours = atof(argv[17]); std::cout << ", runHours: " << runHours;
        evoCondition = argv[18]; std::cout << ", EvoCondition: " <<
            evoCondition;
        growthRate = atof(argv[19]); std::cout << ", growthRate: " <<
            growthRate;
        numThreads = atoi(argv[20]); std::cout << ", numThreads: " <<
            numThreads;
        mu = atof(argv[21]); std::cout << ", mu: "  << mu;
        gammaD = atof(argv[22]); std::cout << ", gammaD: " << gammaD;
        lambda = atof(argv[23]); std::cout << ", lambda: " << lambda;
        beta = atof(argv[24]); std::cout << ", beta: " << beta;
