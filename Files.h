...
// Updated file output for evolution code
std::string SetRawName(int numNodes){
    using namespace Parameters;

    return  "Number" + to_string(Number) + "N" + to_string(numNodes) +
            "NumChanges" + to_string(numChanges) + 
            "AvgDeg" + to_string(avgdeg) + "Seed" + to_string(SingleSeed) +
            "Merit" + evoCondition +
            "InitialDistribution" + initialDistribution +
            "Lambda" + to_string(lambda) + "Beta" + to_string(beta) + 
...
std::string SetRawName(int NNodes = Parameters::N) {
	...
	+ std::to_string(NNodes)  + "Number" + std::to_string(int(Number)) + 
        "R" + to_string(R) + "nd" + std::to_string(nd) + "Alpha" + 
        to_string(alpha) + "AvgDeg" + std::to_string(avgdeg) + 
        MortalityCondition + Topology + "SingleSeed" + 
        std::to_string(SingleSeed) + NetworkType + "pA" + 
        to_string(p_assortativity) + "Merit" + evoCondition + "pRewire" +
        to_string(pRewire) + "GrowthRate" + to_string(growthRate) + 
        "NumThreads" + to_string(numThreads) + "gammaD" + to_string(gammaD)
        + "mu" + to_string(mu) + "lambda" + to_string(lambda) +
        "beta" + to_string(beta);
